...
const EditQuiz = () => {
    ...
    const { data: quiz, isSuccess, isLoading } = useGetQuizQuery(id);
    const { data: videos } = useGetVideosQuery();
    const [editQuiz, { data: editedData }] = useEditQuizMutation();
    // get quiz data which want to edit
    useEffect(() => {
        if (isSuccess) {
            const { question, options, video_id, video_title } = quiz;
            setQuestion(question);
            setVideoInfo({ video_id, video_title });
            setOptions(options);
        }
    }, [isSuccess]);
